...
int main( int argc, char* argv[] )
{
	...
	for(unsigned int i = 0; i < NUM_JOINTS; i++)
	{
		...
		std::string buf = jointNames[i].c_str();
		std::cout << buf << " X" << std::endl;
		controls[i*3] = ModelerControl(buf.c_str(), -M_PI, M_PI, 0.1f, 0);
		//sprintf(buf, "%s Y", jointNames[i].c_str());
		std::cout << buf << " Y" << std::endl;
		controls[i*3+1] = ModelerControl(buf.c_str(), -M_PI, M_PI, 0.1f, 0);
		//sprintf(buf, "%s Z", jointNames[i].c_str());
		std::cout << buf << " Z" << std::endl;
		controls[i*3+2] = ModelerControl(buf.c_str(), -M_PI, M_PI, 0.1f, 0);
	}
